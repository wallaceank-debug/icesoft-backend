...
pool.connect()
    ...
    .then(async () => {
        ...
        await pool.query("ALTER TABLE vendas ALTER COLUMN status TYPE TEXT"); // 👇 NOVO: Remove o limite de 20 letras do status para o Robô do Pix conseguir escrever
        ...
        // 👇 NOVO: Limites personalizados de grupos por produto (Estilo iFood)
        await pool.query("ALTER TABLE produtos ADD COLUMN IF NOT EXISTS limites_grupos JSONB DEFAULT '{}'");
...
app.get('/api/relatorios/funil', async (req, res) => {
    try {
       ...
       // 5. Vendas Reais (Apenas Online - Ignora PDV, Mesas e Apps Terceiros)
        const vendas = await pool.query(`SELECT COUNT(*) FROM vendas WHERE status NOT ILIKE '%cancelad%' AND origem NOT ILIKE '%Balcão%' AND origem NOT ILIKE '%WhatsApp / Telefone%' AND origem NOT ILIKE '%Mesas%' AND origem NOT ILIKE '%iFood%' AND origem NOT ILIKE '%Aiqfome%' ${filtroSQL}`, params);
